...
function getVenueInformation(city) {
    ...
    $.when(
        ...
        if (errorResp.status === 400) {
            $("#messageBox").append(`<p>Venue Error: No venue information found for "${cityName}", please try again </p>`);
            $("#venueTitle").empty()
            $("footer").css("position", "absolute")
        } else {
            $("#messageBox").append(`<p>Venue Error: ${errorResp.responseJSON.message},please try again</p>`);
            $("#venueTitle").empty()
            $("footer").css("position", "absolute")
        }
...
$(document).ready(function () {
    ...
    $("#buttonS").on("click", function (e) {
        ...
        if (cityName === "") {
            ...
            setTimeout(function(){$("footer").css("visibility", "visible");}, 1000);
        }
